...
const directories = fs
  .readdirSync(lootTablePath, { withFileTypes: true })
  .filter(dir => dir.isDirectory())
  .map(dir => dir.name);

const curves = directories.reduce(
  (acc, dir) => {
...
Object.entries(curves).forEach(([key, value]) => {
  describe.skip(`Game Simulation — ${key}`, () => {
    ...
    Object.entries(data).forEach(([source, blocks]) => {
      describe(`Using ${source} block data`, () => {
        it('should simulate games', () => {
          ...
          console.log(
            `[${source}] ${key} - Min: ${minSurvived}, Max: ${maxSurvived}, Avg: ${avgSurvived}, Completion: ${completion}/${totalSimulations} (${((completion / totalSimulations) * 100).toFixed(2)}%)`
          );
        });
        ...
        it('should have 97% survival rate at 1x', () => {
          ...
          console.log(
            `[${source}] ${key} - Survival rate at ${Number(multipliers[target]) / 1e6}x: ${(survivalRate * 100).toFixed(2)}%`
          );

...
import { validateSession } from '@/lib/session-keys/validate-session';
...
import type { SupportedChain } from '@/lib/session-keys/chain';
...
export const useAbstractSession = (chain: SupportedChain) => {
  ...
  return {
    getStoredSession: () => getStoredSession(client, address, chain, createSessionAsync),
    validateSession: (sessionHash: `0x${string}`) =>
      validateSession(client, address, sessionHash, chain, createSessionAsync),
    createAndStoreSession: () => createAndStoreSession(address, createSessionAsync),
